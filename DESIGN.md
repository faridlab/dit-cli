...
Plus `notify` (file watcher) to see writes from *other processes* — the multi-actor case (ADR 0015): a CLI run by each lane must still light up a browser watching the board. Realized as `dit_core::watch::spawn(Arc<Mutex<Dit>>) -> Receiver<()>` (ADR 0017; §16.2's `subscribe()`), bridged by the server into its existing announce channel. The watcher never reads working files: on a debounced burst it compares `HEAD` against the last head it signalled — in memory, **not** the index watermark, because the cache is shared between processes and another actor's CLI absorbs its own commit into it, which would silence the watcher exactly when a browser waits for a frame. When HEAD moved, it reindexes from git blobs only if the shared index is stale (a raw `git commit` never absorbs), then signals once per burst; an own-process write may add one further coalesced frame — a refetch hint, never a loop. `notify` is a dependency of `dit-core` only (8.2.0, recorded here per the no-silent-dependency rule; on macOS the kqueue backend is used rather than FSEvents, which coalesces and can drop single-file modify events — the exact signal the watcher exists for; dit-core is not in the wasm32 check set), and any watcher error — including `MaxFilesWatch` — degrades to a 2 s HEAD-polling loop doing the same seen-head check.
