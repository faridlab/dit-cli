...
fn creation_emits_one_event_per_field_with_no_parent() {
    ...
    let events = walk_field_events(&repo, None, dit_model::DataLayout::DotDir).unwrap();
...
fn a_watermark_limits_the_walk_to_newer_commits() {
    ...
    let full = walk_field_events(&repo, None, dit_model::DataLayout::DotDir).unwrap();
    assert!(full.len() > 2, "creation + two edits: {full:#?}");

    let incremental =
        walk_field_events(&repo, Some(&second), dit_model::DataLayout::DotDir).unwrap();
...
#[test]
fn root_layout_reads_readme_bodies_and_ignores_doc_pages() {
    // ADR 0005 + 0006 walker fixture: issues live at the tree root with
    // README.md bodies; a docs page that is also a README.md must not be
    // mistaken for an issue, and the generated issues/README.md index
    // (ADR 0008) is likewise not an issue body.
    let tmp = tempfile::tempdir().unwrap();
    let repo = hermetic_repo(tmp.path());
    write(
        &tmp.path().join("issues/2026/08/one/README.md"),
        &issue("01K3M9ZXQ2R7VN8P4TDBCEFGHJ", "Login timeout", "todo", "p2"),
    );
    write(
        &tmp.path().join("docs/ARCHITECTURES/README.md"),
        "---\ntitle: Notes on auth\n---\n\nNot an issue.\n",
    );
    write(
        &tmp.path().join("issues/README.md"),
        "<!-- generated by dit 0.1.1 — do not edit; run dit docs build --index -->\n\n# Issues\n",
    );
    repo.add("issues").unwrap();
    repo.add("docs").unwrap();
    repo.commit("create issue + docs").unwrap().unwrap();

    let events = walk_field_events(&repo, None, dit_model::DataLayout::Root).unwrap();
    let ids: Vec<&str> = events.iter().map(|e| e.issue_id.as_str()).collect();
    assert!(
        ids.iter().all(|id| *id == "01K3M9ZXQ2R7VN8P4TDBCEFGHJ"),
        "only the issue body contributes events: {events:#?}"
    );
    assert!(!events.is_empty());

    // The dotdir walker must not read root-layout data either — layouts do
    // not cross.
    let none = walk_field_events(&repo, None, dit_model::DataLayout::DotDir).unwrap();
